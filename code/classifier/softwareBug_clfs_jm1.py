...
csv = r'/content/drive/MyDrive/softwareBugPred/bugPridiction/data/jm1.csv'
missing_values = ['?']
dataset = pandas.read_csv(csv,na_values = missing_values)
# df=dataset.iloc[:,16:20]
# dataset= pandas.DataFrame(df)
# dataset.fillna(0)
# print(dataset.info())

# nan_vl=dataset['uniq_Op','uniq_Opnd','total_Op','total_Opnd','branchCount']
# dataset[nan_vl]=dataset[nan_vl].fill(0)
dataset['uniq_Op']=dataset['uniq_Op'].fillna(0)
dataset['uniq_Opnd']=dataset['uniq_Opnd'].fillna(0)
dataset['total_Op']=dataset['total_Op'].fillna(0)
dataset['total_Opnd']=dataset['total_Opnd'].fillna(0)
dataset['branchCount']=dataset['branchCount'].fillna(0)
X=dataset.iloc[:,0:20]

# dataset.iloc[:,1:20]=dataset.iloc[:,16:21].fillna(0)
...
clf=GradientBoostingClassifier(loss='deviance', max_features='sqrt',n_estimators=100, learning_rate=0.1, max_depth=8, random_state=0)
...
ax.set_title('Results of the experimented Classifiers for JM1 dataset')
ax.set_xticks(x)
